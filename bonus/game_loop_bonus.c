...
/*   Updated: 2025/03/14 22:58:48 by oimzilen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	init_images(t_game *g)
{
	...
	anim_fail = init_frames(g);
	z_fail = init_zombies(g);
	if (!g->blocks.grass || !g->blocks.cobble || !g->blocks.cobble_t
		|| !g->blocks.catr || !g->blocks.catl || !g->blocks.catu
		|| !g->blocks.catd || !g->blocks.food || !g->blocks.exit_c
		|| !g->blocks.exit_o || anim_fail == -1 || z_fail == -1)
	{
		print_error(IMAGE);
		clean_up(g);
	}
